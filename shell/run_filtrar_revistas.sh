...
docker run -it \
  ...
  -w /app \
  lector_gdrive \
  python -u lector_gdrive/filtrar_revistas.py

# Verificar que se creó el archivo de salida
if [ -f "salida/salida.revistas.json" ]; then
    echo ""
    ...
    if command -v jq &> /dev/null; then
        REVISTA_COUNT=$(jq length salida/salida.revistas.json 2>/dev/null || echo "?")
        echo "📊 Cantidad de artículos en revistas procesados: $REVISTA_COUNT"
    fi
    
    FILE_SIZE=$(du -h salida/salida.revistas.json | cut -f1)
